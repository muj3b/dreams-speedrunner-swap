...
import com.example.speedrunnerswap.integration.VoiceChatIntegration;
...
public final class SpeedrunnerSwap extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getServer().getPluginManager().registerEvents(new com.example.speedrunnerswap.task.TaskEventListener(this),
                this);
    ...
    private void validatePowerUpConfig() {
        ...
        for (String s : configManager.getGoodPowerUps())
            check.accept(s);
        for (String s : configManager.getBadPowerUps())
            check.accept(s);
        if (!invalid.isEmpty()) {
    ...
    public void setCurrentMode(SwapMode mode) {
        ...
        // Optionally apply the per-mode default interval when switching modes (if
        // enabled)
        try {
            if (configManager != null && gameManager != null && configManager.getApplyDefaultOnModeSwitch()
